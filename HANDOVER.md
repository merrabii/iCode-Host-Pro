...
## Current state (Phase 4 — COMPLETE and owner-validated 2026-08-31)
...
- **Audit journal (Phase 4, ADR-019)**: table `AuditLog` (migration `init_audit`) — append-only, ADMIN read (`GET /api/audit`, paginated+filtered). Emission côté service (best-effort) pour users/products/servers/auth (register/login/refresh/logout + promote/demote/activate/deactivate + product/server CRUD). `AuditModule` `@Global`. Web **`/manager/journal`**. No update/delete endpoints.
...
Reported decisions: ADR-001..005 + 011..014 APPROVED (Phase 0); ADR-015 + 016 APPROVED (Phase 1); **ADR-017 APPROVED (Phase 2)**; **ADR-018 APPROVED (Phase 3)**; **ADR-019 APPROVED (Phase 4)**; ADR-006 full, 007, 008, 009, 010 stay PROPOSED.
