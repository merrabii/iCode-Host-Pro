...
- **ADR-038 ADDENDUM 2 — 2026-09-15 — réconciliation Deployment↔Order après preuve réelle (COMMIT 4)** : après preuve produit réelle (Order ACTIVE + HTTP 200 Express/hérokû), la row Deployment restait **DEPLOYING** à l'arrêt. Audit machine d'état : le proof-gate store (`awaitAppReady` vrai) prouve réellement la mise en ligne mais ne passait **jamais** la row Deployment en ACTIVE ; seule la reſconciliation lazy du dashboard client (`refreshStatus`) le faisait, si vue. ⇒ décision **correction ciblée** : `reconcileDeploymentActive(orderId)` appelé dans la branche `ready===true` après l'activation de l'Order — `deployment.updateMany({ where:{orderId,status:DEPLOYING}, data:{status:ACTIVE} })` + audit `provision.deployment_active`. **Sémantique DURABLE** : « Order ACTIVE et Deployment ACTIVE représentent le même succès de provisioning, pour LE Deployment concerné, quand le proof-gate est satisfait ». Pas de « Order ACTIVE ⇒ toutes ses Deployments ». Idempotent, jamais FAILED→ACTIVE sans nouvelle preuve, jamais de création, conditionnel donc sûr en concurrence avec `refreshStatus`. ADR-037/038, port, build-pack, proof-gate **inchangés** ; aucun statut forcé en base. 7 tests unit dédiés (TEST1–TEST7), unit **405/405**, tsc vert.

## ADR-039 — Activation Turnstile partagée : `active = enabled && configured` (Phase 3, 2026-09-16)
**Status: IMPLEMENTED** (commit local `fix(security): align turnstile runtime with admin setting`, non poussé).

- **Problème (2 classes)** : (1) `public-config` exposait la clé SITE Turnstile **sans condition** — flag OFF ou config incomplète ⇒ Turnstile « non actif » annoncé/servi comme actif ; (2) **divergence frontend/backend** : le frontend rend le widget sur `turnstileSiteKey !== ''` (sans token) mais `auth.login` et `support-codes.redeem` gated sur le **flag seul** `isTurnstileEnabled()` ⇒ `enabled=true` + config incomplète ⇒ **login/redeem impossibles** (le backend exige un token que le frontend ne peut produire).
- **Décision (invariant durable)** : une **notion effective unique** d'activation, `TurnstileService.isActive()` = `enabled (flag admin) && configured (clés SITE + SECRET présentes, DB→env)`, est la **source de vérité commune** de l'exposition publique **et** de **tout** gate d'enforcement. `active = enabled && configured`, distinct de `configured` (clés seules) et de `enabled` (flag seul).
- **Impl** : `TurnstileService.isActive()` + `getSiteKey()` (résolution unique DB→env, la résolution dupliquée du contrôleur est supprimée) ; `public-config.controller` ne sert la clé SITE **que si active** (secret jamais exposé) ; **`auth.service.login` et `support-codes.controller.redeem` gated sur `isActive()`** (alignement — les deux utilisaient le flag seul).
- **Sémantique fail-closed préservée** : un échec de `verify()` Cloudflare externe rejette (inchangé). « Inactif » signifie **uniquement** configuration absente/incomplète, jamais un échec réseau.
